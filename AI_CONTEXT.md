...
- **Offline-First Strategy**:
  ...
  - **Source of Truth Rule**: `SyncQueue` stores the *intent* and *operation parameters*, but the **Local Database (Drift)** is the ultimate source of truth before sync execution.
  - **Late Binding Strategy**: For complex aggregates (e.g., `DamageReport`), `BackgroundSyncService` reloads the fresh state from the database immediately before calling the API. This ensures that items added while the record was pending sync are included in the final payload (resolving the "Stale Snapshot" problem).
...
### Synchronization Technical Debt
- **Duplicated Mapping**: Currently, `BackgroundSyncService` and repositories maintain separate mapping logic from local entities to domain models/DTOs to avoid circular dependencies. Future refactoring should consolidate these into a shared `DataMapper` or extension methods once the core sync hardening is complete.
