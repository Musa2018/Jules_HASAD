...
namespace Hasad.Application.Features.Assistances.Commands.ApproveAssistance;

public record ApproveAssistanceCommand(Guid Id, decimal ApprovedAmount, string Remarks, string RowVersion) : IRequest<Result<AssistanceDto>>;

public class ApproveAssistanceCommandHandler : IRequestHandler<ApproveAssistanceCommand, Result<AssistanceDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public ApproveAssistanceCommandHandler(IApplicationDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<Result<AssistanceDto>> Handle(ApproveAssistanceCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!AssistanceStatus.CanTransition(assistance.Status, AssistanceStatus.Approved))
            return Result<AssistanceDto>.Failure(new[] { $"Invalid status transition from {assistance.Status} to Approved." });

        var previousStatus = assistance.Status;
        assistance.Status = AssistanceStatus.Approved;
        ...
        _context.AssistanceAuditLogs.Add(new AssistanceAuditLog
        {
            ...
            NewStatus = AssistanceStatus.Approved,
