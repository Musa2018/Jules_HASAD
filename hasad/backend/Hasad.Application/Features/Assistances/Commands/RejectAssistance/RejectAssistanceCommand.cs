...
namespace Hasad.Application.Features.Assistances.Commands.RejectAssistance;

public record RejectAssistanceCommand(Guid Id, string Remarks, string RowVersion) : IRequest<Result<AssistanceDto>>;

public class RejectAssistanceCommandHandler : IRequestHandler<RejectAssistanceCommand, Result<AssistanceDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public RejectAssistanceCommandHandler(IApplicationDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<Result<AssistanceDto>> Handle(RejectAssistanceCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!AssistanceStatus.CanTransition(assistance.Status, AssistanceStatus.Rejected))
            return Result<AssistanceDto>.Failure(new[] { $"Invalid status transition from {assistance.Status} to Rejected." });

        var previousStatus = assistance.Status;
        assistance.Status = AssistanceStatus.Rejected;
        ...
        _context.AssistanceAuditLogs.Add(new AssistanceAuditLog
        {
            ...
            NewStatus = AssistanceStatus.Rejected,
