...
namespace Hasad.Application.Features.Compensations.Commands.MarkAsPaidCompensation;

public record MarkAsPaidCompensationCommand(Guid Id, string Remarks, string RowVersion) : IRequest<Result<CompensationDto>>;

public class MarkAsPaidCompensationCommandHandler : IRequestHandler<MarkAsPaidCompensationCommand, Result<CompensationDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public MarkAsPaidCompensationCommandHandler(IApplicationDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<Result<CompensationDto>> Handle(MarkAsPaidCompensationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!CompensationStatus.CanTransition(compensation.Status, CompensationStatus.Paid))
            return Result<CompensationDto>.Failure(new[] { $"Invalid status transition from {compensation.Status} to Paid." });
        ...
        _context.CompensationAuditLogs.Add(new CompensationAuditLog
        {
            ...
            NewStatus = CompensationStatus.Paid,
