...
namespace Hasad.Application.Features.Compensations.Commands.SubmitCompensation;

public record SubmitCompensationCommand(Guid Id, string RowVersion) : IRequest<Result<CompensationDto>>;

public class SubmitCompensationCommandHandler : IRequestHandler<SubmitCompensationCommand, Result<CompensationDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public SubmitCompensationCommandHandler(IApplicationDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<Result<CompensationDto>> Handle(SubmitCompensationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!CompensationStatus.CanTransition(compensation.Status, CompensationStatus.Submitted))
            return Result<CompensationDto>.Failure(new[] { $"Invalid status transition from {compensation.Status} to Submitted." });

        var previousStatus = compensation.Status;
        compensation.Status = CompensationStatus.Submitted;
        ...
        _context.CompensationAuditLogs.Add(new CompensationAuditLog
        {
            ...
            NewStatus = CompensationStatus.Submitted,
